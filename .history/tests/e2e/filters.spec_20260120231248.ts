...
test.describe('Filters and Sorting', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const vehicles = [
      ...
      { type: 'motorcycle', nickname: 'Speed Bike', mileage: '15000', wheels: '2', seatStatus: 'works', engineStatus: 'works' },
    ...
    for (const vehicle of vehicles) {
      ...
      if (vehicle.type === 'sedan' || vehicle.type === 'coupe' || vehicle.type === 'mini-van') {
        await page.selectOption('select[name="doors"]', (vehicle as any).doors);
      }
      
      if (vehicle.type === 'motorcycle') {
        await page.selectOption('select[name="seatStatus"]', (vehicle as any).seatStatus);
      }
      
      await page.selectOption('select[name="engineStatus"]', vehicle.engineStatus);
      await page.click('button[type="submit"]');
      // Use .first() to handle the current toast
