...
describe('Vehicle CRUD Integration', () => {
  ...
  describe('data integrity', () => {
    ...
    it('preserves registration status through save/load', () => {
      // Create vehicle with successful registration
      const highMileageSedan = service.createVehicle(
        { type: 'sedan', nickname: 'Old Sedan', mileage: 150000 },
        []
      );

      expect(highMileageSedan.ok).toBe(true);
      if (!highMileageSedan.ok) return;

      expect(highMileageSedan.value.registration.status).toBe('registered');

      // Save and load
      storage.save([highMileageSedan.value]);
      const loaded = storage.load();
      ...
      expect(loadedVehicle?.registration.status).toBe('registered');
      if (loadedVehicle?.registration.status === 'registered') {
        expect(loadedVehicle.registration.registrationId).toBeDefined();
        expect(loadedVehicle.registration.registrationId).toMatch(/^SEDAN-/);
      }
    });
