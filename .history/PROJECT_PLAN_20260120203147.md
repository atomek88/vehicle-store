...
- **Tasks:**
  - [ ] Create type-specific field components (`SedanFields.tsx`, etc.) per technical spec.
  - [ ] Create the main `VehicleForm.tsx` component with fields: nickname, mileage, vehicle type selector, and type-specific fields (wheels, doors, etc.).
...
- **Tasks:**
  ...
  - [ ] Display vehicle columns: Type, Nickname, Mileage, Registration ID/Status, Engine Status, Actions.
...
- **Tasks:**
  ...
  - [ ] Display vehicle fields per technical spec: type, nickname, mileage, wheels, doors (or type-specific fields), engine status, and registration status.
  - [ ] Show registration as a single immutable value (success with ID or failed with error message).
...
- **Tasks:**
  - [ ] Create the registration log page at `src/app/registration-log/page.tsx`.
  - [ ] Display vehicles with columns: Registration ID (or error), Vehicle (nickname + type), Status, Timestamp (createdAt).
  - [ ] Implement filtering by registration status (Success/Failed/Pending if applicable).
  - [ ] Add search functionality by vehicle nickname or registration ID.
  - [ ] Reuse existing vehicle state/context - no new data model needed.
