...
### Story 4.6: CRUD E2E Tests ✅ COMPLETED

- **Description:** Use Playwright to conduct E2E tests for the complete create, read, update, and delete user journeys.
- **Status:** All 17 E2E tests passing. Comprehensive coverage of create, edit, delete flows with proper test isolation, toast notifications, and dialog interactions.
- **Tasks:**
  ...
  - [x] Fix test isolation with localStorage clearing.
  - [x] Implement row-scoped selectors for reliable element targeting.
  - [x] Add proper timing waits for toast notifications and modal interactions.

### Story 4.7: Filtering & Sorting E2E Tests ✅ COMPLETED

- **Description:** Use Playwright to test the dashboard's interactive features.
- **Status:** All 14 E2E tests passing. Comprehensive coverage of search, filtering by type/engine/registration status, sorting by nickname/mileage/date, and combined filter+sort scenarios. Uses localStorage pre-seeding for fast test execution.
- **Tasks:**
  - [x] Create `tests/e2e/filters.spec.ts`.
  - [x] Write tests to verify that search, filter, and sort controls correctly update the vehicle list.
  - [x] Implement robust test data seeding via localStorage.
  - [x] Fix test selectors to match actual DOM structure.
  - [x] Ensure reliable test execution with proper wait conditions.
...
### Story 5.1: Test Hardening & Refinement ✅ COMPLETED

- **Description:** Review and improve existing tests for reliability and coverage.
- **Status:** All 31 E2E tests passing reliably. Fixed strict mode violations, removed unnecessary timeouts, improved toast lifecycle management, and cleaned up debug logging.
- **Tasks:**
  - [x] Review all E2E tests and add assertions for toast notifications.
  - [x] Fix strict mode violations by using `.first()` for toast locators.
  - [x] Replace slow toast wait timeouts with faster modal close waits.
  - [x] Remove debug logging from test files.
  - [x] Ensure consistent test patterns across all E2E test files.

### Story 5.2: Cross-Cutting & Exploratory Testing ✅ COMPLETED

- **Description:** Implement accessibility improvements and create testing documentation.
- **Status:** All accessibility improvements implemented. Created comprehensive audit, implemented high-priority fixes (enhanced focus indicators, toast ARIA alerts, table caption, contextual ARIA labels), and created detailed testing guides for future manual testing.
- **Tasks:**
  - [x] Create comprehensive accessibility audit document (ACCESSIBILITY_AUDIT.md).
  - [x] Implement enhanced focus indicators for keyboard navigation.
  - [x] Add role="alert" and aria-live to toast notifications.
  - [x] Add table caption for screen reader context.
  - [x] Enhance ARIA labels on action buttons with vehicle context.
  - [x] Create ACCESSIBILITY_IMPROVEMENTS.md summary document.
  - [x] Create RESPONSIVE_TESTING_GUIDE.md for future testing.
...
### Story 6.1: Responsive Design & State Refinements ✅ COMPLETED

- **Description:** Ensure the application is fully responsive and provides clear feedback on its state.
- **Status:** All features implemented. Loading states show during hydration, error recovery UI displays hydration errors with messages, responsive layouts use Tailwind utilities (grid, flexbox, overflow-x-auto for tables).
- **Tasks:**
  - [x] Add loading states (e.g., spinners) for data hydration and form submissions.
  - [x] Implement a graceful error recovery UI for data hydration failures.
  - [x] Finalize the responsive layouts for the table, filters, and forms.

### Story 6.2: Notifications & User Feedback ✅ COMPLETED

- **Description:** Implement clear user feedback for all primary actions.
- **Status:** All features implemented and tested. Toast notifications work for all CRUD actions, form validation shows error messages, confirmation dialog implemented for delete actions.
- **Tasks:**
  - [x] Integrate the `useToast` hook to show success/error notifications for all CRUD actions.
  - [x] Ensure form-level error banners are shown for business rule violations (e.g., nickname taken).
  - [x] Implement and connect the confirmation dialog for the delete action.

---

## EPIC 7: Manual Testing & Validation (Future)

> **Goal:** Perform comprehensive manual testing, automated accessibility scans, and responsive design validation before production deployment.

### Story 7.1: Automated Accessibility Testing

- **Description:** Run automated accessibility testing tools to identify and fix any remaining a11y issues.
- **Tasks:**
  - [ ] Install and run axe DevTools browser extension on all pages.
  - [ ] Run Lighthouse accessibility audit in Chrome DevTools.
  - [ ] Run WAVE accessibility checker.
  - [ ] Document and fix any critical or high-priority issues found.
  - [ ] Verify WCAG 2.1 Level AA compliance.

### Story 7.2: Responsive Design Validation

- **Description:** Manually test responsive layouts across all target device sizes.
- **Tasks:**
  - [ ] Test at mobile breakpoints (320px, 375px, 414px, 768px).
  - [ ] Test at tablet breakpoints (768px, 834px, 1024px).
  - [ ] Test at desktop breakpoints (1024px, 1280px, 1440px, 1920px).
  - [ ] Verify table scrolling and layout on mobile devices.
  - [ ] Test forms and modals at all breakpoints.
  - [ ] Verify touch targets meet minimum size requirements (44x44px).
  - [ ] Test on real iOS and Android devices if possible.

### Story 7.3: Exploratory Testing & Edge Cases

- **Description:** Perform manual exploratory testing to discover edge cases and usability issues.
- **Tasks:**
  - [ ] Test with very long vehicle nicknames and registration IDs.
  - [ ] Test with maximum and minimum mileage values.
  - [ ] Test creating many vehicles (50+) to verify performance.
  - [ ] Test rapid-fire actions (quick create/delete cycles).
