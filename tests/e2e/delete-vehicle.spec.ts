...
test.describe('Delete Vehicle Flow', () => {
  ...
  test('should delete vehicle from details view', async ({ page }) => {
    ...
    // Click delete button (button text is now just "Delete")
  ...
  test('should delete multiple vehicles sequentially', async ({ page }) => {
    ...
    for (const name of vehicles) {
      ...
      await expect(page.locator('text=Vehicle created successfully').first()).toBeVisible();
      // Wait for modal to close
      await page.waitForSelector('text=Create New Vehicle', { state: 'hidden', timeout: 5000 });
      // Wait for toast to disappear before creating next vehicle
      await page.locator('text=Vehicle created successfully').first().waitFor({ state: 'hidden', timeout: 6000 });
    }

    // Delete first vehicle
    const row1 = page.locator('tr:has-text("Vehicle One")');
    await row1.getByRole('button', { name: /Delete Vehicle One/ }).click();
    await expect(page.getByRole('heading', { name: 'Delete Vehicle' })).toBeVisible();
    await page.locator('dialog, [role="dialog"]').getByRole('button', { name: 'Delete', exact: true }).click();
    await expect(page.locator('text=Vehicle deleted successfully')).toBeVisible();
    ...
    const row2 = page.locator('tr:has-text("Vehicle Two")');
    await row2.getByRole('button', { name: /Delete Vehicle Two/ }).click();
    await expect(page.getByRole('heading', { name: 'Delete Vehicle' })).toBeVisible();
    await page.locator('dialog, [role="dialog"]').getByRole('button', { name: 'Delete', exact: true }).click();
    await expect(page.locator('text=Vehicle deleted successfully').first()).toBeVisible();
  ...
  test('should show empty state after deleting all vehicles', async ({ page }) => {
    ...
    // Delete it
    const row = page.locator('tr:has-text("Last Vehicle")');
    await row.getByRole('button', { name: /Delete Last Vehicle/ }).click();
    await expect(page.getByRole('heading', { name: 'Delete Vehicle' })).toBeVisible();
    await page.locator('dialog, [role="dialog"]').getByRole('button', { name: 'Delete', exact: true }).click();
    await expect(page.locator('text=Vehicle deleted successfully')).toBeVisible();
