...
describe('useVehicles', () => {
  ...
  describe('createVehicle', () => {
    it('creates a new vehicle successfully', async () => {
      ...
      let createResult: { ok: true; vehicle: Vehicle } | { ok: false; error: string } | undefined;
    ...
    it('rejects duplicate nicknames', async () => {
      ...
      let duplicateResult: { ok: true; vehicle: Vehicle } | { ok: false; error: string } | undefined;
      ...
      if (duplicateResult && !duplicateResult.ok) {
    ...
    it('persists vehicles to storage after creation', async () => {
      ...
      await waitFor(() => {
        const calls = vi.mocked(storage.storageRepository.save).mock.calls;
        // Should have at least 2 calls: initial hydration + after create
      ...
      const calls = vi.mocked(storage.storageRepository.save).mock.calls;
      const lastCall = calls[calls.length - 1];
      expect(lastCall?.[0]?.[0]?.nickname).toBe('Bike');
    });
  });
  ...
  describe('updateVehicle', () => {
    it('updates an existing vehicle', async () => {
      ...
      let updateResult: { ok: true; vehicle: Vehicle } | { ok: false; error: string } | undefined;

      await act(async () => {
        updateResult = await result.current.updateVehicle(mockVehicles[0]!.id, {
          type: 'sedan',
      ...
      if (updateResult && updateResult.ok) {
        expect(updateResult.vehicle.nickname).toBe('Updated Sedan');
        expect(updateResult.vehicle.mileage).toBe(60000);
      }

      expect(result.current.state.vehicles[0]?.nickname).toBe('Updated Sedan');
    ...
    it('returns error for non-existent vehicle', async () => {
      ...
      await act(async () => {
        updateResult = await result.current.updateVehicle('non-existent-id', {
          type: 'sedan',
          nickname: 'Test',
          mileage: 10000,
        });
  ...
  describe('deleteVehicle', () => {
    it('deletes an existing vehicle', async () => {
      ...
      let deleteResult: { ok: true } | { ok: false; error: string } | undefined;
    ...
    it('returns error for non-existent vehicle', async () => {
      ...
      await act(async () => {
        deleteResult = await result.current.deleteVehicle('non-existent-id');
      });
