...
const MiniVanPersistedBaseSchema = VehicleBaseSchema.extend({
  type: z.literal('mini-van'),
  wheels: CarWheelsSchema,
  doors: SedanDoorsSchema,
...
export const MiniVanPersistedSchema = MiniVanPersistedBaseSchema;
...
export const VehiclePersistedSchema = z.discriminatedUnion('type', [
  ...
  MiniVanPersistedBaseSchema,
  MotorcyclePersistedSchema,
]).superRefine((val, ctx) => {
  // Validate mini-van doorConfig length matches doors count
  if (val.type === 'mini-van' && val.doorConfig.length !== val.doors) {
    ctx.addIssue({
